...
def _fwd_kernel(Q, K, V, sm_scale,  #
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        qk += tl.dot(q, k, f32_backend="tf32")
        ...
        acc *= alpha[:, None]
        acc += tl.dot(p.to(V.dtype.element_ty), v, f32_backend="tf32")
        # -- update m_i and l_i --
...
def _bwd_kernel_one_col_block(Q, K, V, sm_scale, qk_scale,  #
    ...
    for start_m in range(lo, num_block * BLOCK_M, BLOCK_M):
        ...
        dv += tl.dot(tl.trans(p.to(Q.dtype.element_ty)), do, f32_backend="tf32")
        # compute dp = dot(v, do)
        Di = tl.load(D_ptrs + offs_m_curr)
        # dp = tl.zeros([BLOCK_M, BLOCK_N], dtype=tl.float32) - Di[:, None]
        dp = tl.dot(do, tl.trans(v), f32_backend="tf32")
        # compute ds = p * (dp - delta[:, None])
        ds = (p * (dp - Di[:, None]) * sm_scale).to(Q.dtype.element_ty)
        # compute dk = dot(ds.T, q)
        dk += tl.dot(tl.trans(ds), q, f32_backend="tf32")
        # compute dq
        if not SEQUENCE_PARALLEL:
            dq = tl.load(DQ_block_ptr)
            dq += tl.dot(ds, k, f32_backend="tf32")
            ...
            if MMA_V3:
                ...
                dq = tl.trans(tl.dot(tl.trans(k), tl.trans(ds), f32_backend="tf32"))
            tl.store(DQ_block_ptr, dq.to(Q.dtype.element_ty))
