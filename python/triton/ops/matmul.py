...
def _kernel(A, B, C, M, N, K,  #
    ...
    for k in range(0, tl.cdiv(K, BLOCK_K * SPLIT_K)):
        ...
        if AB_DTYPE is not None:
            ...
            b = b.to(AB_DTYPE)
        if fp8_fast_accum:
            acc = tl.dot(a, b, acc, out_dtype=acc_dtype, f32_backend=f32_backend)
        else:
            acc += tl.dot(a, b, out_dtype=acc_dtype, f32_backend=f32_backend)
...
class _matmul(torch.autograd.Function):
    ...
    @staticmethod
    def _call(a, b, acc_dtype, f32_backend, fp8_fast_accum, output_dtype):
        device = a.device
        ...
        _kernel[grid](
            ...
            acc_dtype=acc_dtype,  #
            f32_backend=f32_backend,  #
            fp8_fast_accum=fp8_fast_accum,  #
            GROUP_M=8, AB_DTYPE=ab_dtype)
        return c

    @staticmethod
    def forward(ctx, a, b, acc_dtype=None, f32_backend="tf32", fp8_fast_accum=True, output_dtype=None):
        return _matmul._call(a, b, acc_dtype=acc_dtype, f32_backend=f32_backend, fp8_fast_accum=fp8_fast_accum,
